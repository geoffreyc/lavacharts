...
describe('lava.js core functions', function() {
    ...
    describe('lava.getChart()', function() {
        ...
        beforeEach(function() {
            lava._charts = [];
            lava._charts.push(
                new lava.Chart("LineChart", "TestChart")
            );
        });

        it('should return a valid chart to the callback.', function() {
            lava.getChart('TestChart', function (chart) {
                expect(chart).toBe(lava.Chart);
                //expect(chart.prop).toEqual('thing');
                //expect(chart.options.title).toEqual('Weather in October');
                //expect(chart.data.cols[0].type).toEqual('date');
            });
        });

        it('should throw a CHART_NOT_FOUND error if the chart is not found.', function() {
            expect(function() {
                lava.getChart('Bee Population', Callback);
            }).toThrowError(lava._errors.ChartNotFound);
        });

        it('should throw an INVALID_LABEL error if a string chart label is not given.', function() {
            expect(function() {
                lava.getChart(1234, Callback);
            }).toThrowError(lava._errors.InvalidLabel);
        });

        it('should throw an INVALID_CALLBACK error if a function callback is not given.', function() {
            expect(function() {
